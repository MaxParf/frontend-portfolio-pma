...
const del = (db, store, key) => new Promise((resolve, reject) => { const request = db.transaction(store, "readwrite").objectStore(store).delete(key); request.onsuccess = () => resolve(); request.onerror = () => reject(request.error); });
...
export function createSandboxStorage({ db, fixture, fixtureVersion, media }) {
  ...
  return {
    ...
    async reset() { await del(db, "state", "saved"); await del(db, "metadata", "fixtureVersion"); await media.clear(); return normalizeProjectState(fixture); },
  };
}
