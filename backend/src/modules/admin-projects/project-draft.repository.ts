...
export class ProjectDraftRepository {
  ...
  async snapshot(projectId: string): Promise<ProjectDraftContent> {
    ...
    const [translations, technologies, media] = await Promise.all([
      ...
      this.pool.query<any>("select m.id,m.external_key,m.path,m.source_type,m.role,pm.orientation,pm.sort_order,mt.locale,mt.alt_text,mt.aria_label from project_media pm join media_assets m on m.id=pm.media_asset_id join media_asset_translations mt on mt.media_asset_id=m.id where pm.project_id=$1 order by pm.orientation nulls last,pm.sort_order", [projectId]),
    ]);
    ...
    for (const item of media.rows) { if (!item.orientation) throw Object.assign(new Error("Project media orientation is unresolved; migrate dimensions before editing this project."), { code: "ORIENTATION_UNRESOLVED" }); const key = item.source_type === "managed" ? item.id : item.external_key; const asset = assets.get(key) ?? (item.source_type === "managed" ? { id: item.id, sourceType: "managed", assetId: item.id, role: item.role, orientation: item.orientation, sortOrder: item.sort_order, translations: {} } : { id: item.external_key, sourceType: "legacy", src: item.path, role: item.role, orientation: item.orientation, sortOrder: item.sort_order, translations: {} }); asset.translations[item.locale] = { alt: item.alt_text, ariaLabel: item.aria_label }; assets.set(key, asset); }
