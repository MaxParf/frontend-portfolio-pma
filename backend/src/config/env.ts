...
const storageEnvSchema = storageInputSchema.transform((value) => {
...
const runtimeEnvSchema = z.object({
  ...
  CORS_ORIGINS: z.string().min(1).transform((value) => value.split(",").map((origin) => origin.trim()).filter(Boolean)).pipe(z.array(z.string().url()).min(1)),
  CMS_ORIGINS: z.string().min(1).transform((value) => value.split(",").map((origin) => origin.trim()).filter(Boolean)).pipe(z.array(z.string().url()).min(1)),
...
const rawEnvSchema = runtimeEnvSchema.and(storageEnvSchema);

const s3ProbeEnvSchema = storageEnvSchema.superRefine((value, ctx) => {
  if (value.NODE_ENV !== "production") {
    ctx.addIssue({ code: "custom", path: ["NODE_ENV"], message: "NODE_ENV must be production for the S3 storage probe." });
  }
  if (value.STORAGE_PROVIDER !== "s3") {
    ctx.addIssue({ code: "custom", path: ["STORAGE_PROVIDER"], message: "STORAGE_PROVIDER must be s3 for the S3 storage probe." });
  }
});

export type AppEnv = z.infer<typeof rawEnvSchema>;
export type StorageEnv = z.infer<typeof storageEnvSchema>;
export type S3ProbeEnv = z.infer<typeof s3ProbeEnvSchema>;

export function loadEnv(source: NodeJS.ProcessEnv = process.env): AppEnv {
  const result = rawEnvSchema.safeParse(source);
  if (!result.success) throw new Error(`Invalid backend environment: ${result.error.issues.map((issue) => `${issue.path.join(".")}: ${issue.message}`).join("; ")}`);
  return result.data;
}

export function loadS3ProbeEnv(source: NodeJS.ProcessEnv = process.env): S3ProbeEnv {
  const result = s3ProbeEnvSchema.safeParse(source);
  if (!result.success) throw new Error(`Invalid S3 probe environment: ${result.error.issues.map((issue) => `${issue.path.join(".")}: ${issue.message}`).join("; ")}`);
  return result.data;
}
