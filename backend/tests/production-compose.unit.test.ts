...
test("production compose separates private, edge, and S3 egress networks", async () => {
  ...
  for (const directory of ["components", "config", "data", "mappers", "services", "images"]) {
    assert.match(publicDockerfile, new RegExp(`COPY --chown=101:101 ${directory} /usr/share/nginx/html/${directory}`));
  }
