...
- **Inputs:** only an explicit Git commit (`PORTFOLIO_CMS_SOURCE_COMMIT`) or the clean worktree `HEAD`. `scripts/build-runtime.sh` records that resolved SHA in the bundle and archives the explicit runtime graph, never the working tree.
